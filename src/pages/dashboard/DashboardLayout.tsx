...
const DashboardLayout = () => {
  ...
  return (
    <div className={isProfileStudio ? "flex h-screen w-full bg-zinc-50 overflow-hidden" : "flex min-h-screen w-full bg-zinc-50"}>
      ...
      <div className={`flex flex-col flex-1 min-w-0 min-h-0 overflow-hidden ${isProfileStudio ? "" : ""}`}>
        ...
        <header className={`flex items-center justify-between px-4 py-3 bg-white border-b border-zinc-200 sticky top-0 z-40 shadow-sm ${isProfileStudio ? "hidden" : "md:hidden"}`}>
        ...
        <main className={isProfileStudio ? "flex-1 min-h-0 overflow-hidden p-0" : "flex-1 p-6 md:p-8 overflow-auto bg-zinc-50"}>
          <Outlet />
        </main>
      </div>
    </div>
