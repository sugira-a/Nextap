...
const MyCard = () => {
    ...
    const load = async () => {
      try {
        const response = await apiRequest<{
          ...
          profile: { public_slug: string } | null;
          card?: any;
        }>("/api/auth/me");
        ...
        const profileSlug = response.profile?.public_slug ?? null;
        setSlug(profileSlug);

        if (!profileSlug) {
          toast.warning("Your profile URL isn't set yet. Go to Edit Profile to set it up.", {
            duration: 5000,
          });
        }
  ...
  const profileUrl = slug ? `${window.location.origin}/u/${slug}` : null;
  const cardUrl = card?.code ? `${window.location.origin}/card/${card.code}` : null;
    ...
    >
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <motion.div
          ...
          className="bg-white border border-zinc-200 rounded-2xl p-8 flex flex-col items-center gap-6 shadow-md hover:shadow-lg transition-shadow"
        >
        ...
        <motion.div
          ...
          className="bg-white border border-zinc-200 rounded-2xl p-8 flex flex-col items-center justify-center gap-5 shadow-md hover:shadow-lg transition-shadow"
        >
          <div className="w-40 h-40 bg-zinc-50 border border-zinc-200 rounded-2xl flex items-center justify-center overflow-hidden">
            {profileUrl ? (
              ...
              <p className="text-xs text-zinc-400 text-center px-3">Set up your profile to generate a QR code</p>
            )}
          </div>
          ...
          <div className="text-center">
            ...
            {slug ? (
              ...
              <p className="text-xs text-red-400 mt-1">No profile URL set</p>
            )}
            {cardUrl && (
          ...
          <div className="flex gap-2 w-full">
            <button
              onClick={() => profileUrl && copyLink(profileUrl, "qr")}
              disabled={!profileUrl}
              className="flex-1 py-2.5 text-sm font-semibold text-zinc-900 border border-zinc-300 rounded-xl hover:bg-zinc-50 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
            >
              {copiedKey === "qr" ? "Copied!" : "Copy Link"}
            </button>
            <button
              onClick={() => slug && window.open(`/u/${slug}`, "_blank")}
              disabled={!slug}
              className="flex-1 py-2.5 text-sm font-semibold text-white bg-zinc-900 rounded-xl hover:bg-zinc-700 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
            >
              Preview →
            </button>
          </div>
        ...
        <div className="divide-y divide-zinc-50">
          {[
            ...
            },
            {
              ...
              label: "Profile",
              value: slug ? `/u/${slug}` : "Not configured",
              mono: true,
            },
          ].map((item) => (
            <div key={item.label} className="flex items-center gap-4 px-6 py-4 hover:bg-zinc-50/60 transition-colors">
              ...
              <div className="min-w-0 flex-1">
                ...
                {item.badge ? (
                  ...
                  <p className={`text-sm font-semibold text-zinc-900 truncate ${item.mono ? "font-mono tracking-wide" : ""}`}>
                    {item.value}
                  </p>
                )}
              </div>
      ...
      {(profileUrl || cardUrl) && (
        ...
        >
          ...
          <div className="divide-y divide-zinc-50">
            {[
              ...(profileUrl ? [{ key: "profile", label: "Profile URL", value: profileUrl, action: () => copyLink(profileUrl, "profile") }] : []),
              ...(cardUrl ? [{ key: "card", label: "Card URL", value: cardUrl, action: () => copyLink(cardUrl, "card") }] : []),
            ].map((row) => (
